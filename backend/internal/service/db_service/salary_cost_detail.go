...
func (s *DatabaseService) ListSalaryCostDetails(salaryCostID int64) ([]models.SalaryCostDetail, error) {
	...
	rows, err := s.db.Query(string(query), salaryCostID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *DatabaseService) CalculateSalaryCostDetails(salaryCostID int64, userID int64) error {
	cost, err := s.GetSalaryCost(userID, salaryCostID)
	if err != nil {
		return err
	}
	salary, err := s.GetSalary(userID, cost.SalaryID)
	if err != nil {
		return err
	}

	// Cleanup First
	err = s.ClearSalaryCostDetails(cost.ID)
	if err != nil {
		return err
	}
	...
	for {
		...
		amountPerMonth := s.CalculateCostAmount(*cost, *salary)
		...
		if totalAmount > 0 {
			...
			_, err := s.UpsertSalaryCostDetails(payload)
			if err != nil {
				return err
			}
		}
...
func (s *DatabaseService) UpsertSalaryCostDetails(payload models.CreateSalaryCostDetail) (int64, error) {
...
func (s *DatabaseService) ClearSalaryCostDetails(salaryCostDetailID int64) error {
...
func (s *DatabaseService) RefreshSalaryCostDetails(userID int64, salaryID int64) error {
	costs, _, err := s.ListSalaryCosts(userID, salaryID, 1, 10000)
	if err != nil {
		return err
	}
	for _, cost := range costs {
		err = s.CalculateSalaryCostDetails(cost.ID, userID)
		if err != nil {
			return err
		}
	}
