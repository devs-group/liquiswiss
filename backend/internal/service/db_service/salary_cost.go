...
func (s *DatabaseService) ListSalaryCosts(userID int64, salaryID int64, page int64, limit int64) ([]models.SalaryCost, int64, error) {
	...
	rows, err := s.db.Query(string(query), salaryID, userID, (page)*limit, 0)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		salaryCost, err := s.GetSalaryCost(userID, salaryCostID)
		if err != nil {
			return nil, 0, err
		}
...
func (s *DatabaseService) GetSalaryCost(userID int64, salaryCostID int64) (*models.SalaryCost, error) {
	...
	err = s.db.QueryRow(string(query), salaryCostID, userID).Scan(
		&salaryCost.ID,
		&labelID,
		&labelName,
		&salaryCost.Cycle,
	...
	costDetails, err := s.ListSalaryCostDetails(salaryCost.ID)
	if err != nil {
		return nil, err
	}
...
func (s *DatabaseService) CreateSalaryCost(payload models.CreateSalaryCost, userID int64, salaryID int64) (int64, error) {
...
func (s *DatabaseService) UpdateSalaryCost(payload models.CreateSalaryCost, userID int64, salaryCostID int64) error {
...
func (s *DatabaseService) DeleteSalaryCost(salaryCostID int64, userID int64) error {
...
func (s *DatabaseService) CopySalaryCosts(payload models.CopySalaryCosts, userID int64, salaryID int64) error {
