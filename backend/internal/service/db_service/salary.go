...
func (s *DatabaseService) ListSalaries(userID int64, employeeID int64, page int64, limit int64) ([]models.Salary, int64, error) {
	...
	for rows.Next() {
		...
		salary, err := s.GetSalary(userID, salaryID)
		if err != nil {
			return nil, 0, err
		}
...
func (s *DatabaseService) GetSalary(userID int64, salaryID int64) (*models.Salary, error) {
	...
	err = s.db.QueryRow(string(query), salaryID, userID).Scan(
		&salary.ID,
		&salary.EmployeeID,
	...
	salaryCosts, _, err := s.ListSalaryCosts(userID, salary.ID, 1, 1000)
	if err != nil {
		return nil, err
	}
	...
	nextExecutionDate := s.CalculateSalaryExecutionDate(salary.FromDate, salary.ToDate, &salary.Cycle, salary.DBDate, 1, true)
...
func (s *DatabaseService) CreateSalary(payload models.CreateSalary, userID int64, employeeID int64) (int64, *int64, *int64, error) {
	tx, err := s.db.Begin()
	if err != nil {
	...
	if previous.ID != 0 {
		...
		if needsAdjustment {
			...
			logger.Logger.Debugf("Adjusting previous salary entry to: %s", newToDate)
			_, err := tx.Exec(`
	...
	if next.ID != 0 {
		...
		if needsAdjustment {
			...
			logger.Logger.Debugf("Adjusting current salary entry to: %s", newToDate)
			_, err = tx.Exec(`
            UPDATE salaries SET to_date = ? WHERE id = ?
        `, newToDate, salaryID)
			if err != nil {
...
func (s *DatabaseService) UpdateSalary(payload models.UpdateSalary, employeeID int64, salaryID int64) (*int64, *int64, error) {
	tx, err := s.db.Begin()
	if err != nil {
		return nil, nil, err
	}
	...
	if previous.ID != 0 {
		...
		previousFromDate := time.Time(previous.FromDate)
		newToDate := utils.GetNextAvailableDate(previousFromDate, currentFromDate, previous.Cycle)
		logger.Logger.Debugf("Adjusting previous salary entry (ID: %d) toDate to: %s", previous.ID, newToDate)
		_, err = tx.Exec(`
				UPDATE salaries SET to_date = ? WHERE id = ?
			`, newToDate, previous.ID)
		if err != nil {
	...
	if next.ID != 0 {
		...
		nextFromDate := time.Time(next.FromDate)
		newToDate := utils.GetNextAvailableDate(currentFromDate, nextFromDate, *payload.Cycle)
		logger.Logger.Debugf("Adjusting current salary entry (ID: %d) toDate to: %s", salaryID, newToDate)
		_, err = tx.Exec(`
				UPDATE salaries SET to_date = ? WHERE id = ?
			`, newToDate, salaryID)
		if err != nil {
...
func (s *DatabaseService) DeleteSalary(toDeleteSalary *models.Salary, userID int64) (*int64, *int64, error) {
	tx, err := s.db.Begin()
	if err != nil {
		return nil, nil, err
	}
