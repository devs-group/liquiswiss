...
CREATE OR REPLACE VIEW ranked_salaries AS
WITH ranked_salary AS (
    SELECT
        ...
        CAST(
            COALESCE(SUM(
                CASE
                    WHEN sc.distribution_type IN ('employee', 'both') AND sc.amount_type = 'percentage'
                        THEN (s.amount * sc.amount / 100000)
                    WHEN sc.distribution_type IN ('employee', 'both') AND sc.amount_type = 'fixed'
                        THEN sc.amount
                    ELSE 0
                END
            ), 0) AS INTEGER
        ...
        CAST(
            COALESCE(SUM(
                CASE
                    WHEN sc.distribution_type IN ('employer', 'both') AND sc.amount_type = 'percentage'
                        THEN (s.amount * sc.amount / 100000)
                    WHEN sc.distribution_type IN ('employer', 'both') AND sc.amount_type = 'fixed'
                        THEN sc.amount
                    ELSE 0
                END
            ), 0) AS INTEGER
