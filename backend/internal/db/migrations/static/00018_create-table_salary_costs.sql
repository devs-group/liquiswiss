...
CREATE TABLE IF NOT EXISTS salary_costs (
    ...
    CONSTRAINT FK_Cost_Label FOREIGN KEY (label_id) REFERENCES salary_cost_labels (id) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT FK_Cost_Salary FOREIGN KEY (salary_id) REFERENCES salaries (id) ON DELETE CASCADE ON UPDATE CASCADE,
...
-- +goose Down
-- +goose StatementBegin
DROP TABLE IF EXISTS salary_costs;
-- +goose StatementEnd
