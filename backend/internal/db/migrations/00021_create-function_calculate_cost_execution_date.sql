...
BEGIN
    ...
    ELSE
        CASE cost_cycle
            WHEN 'daily' THEN
                SET cost_execution_date = DATE_SUB(history_execution, INTERVAL relative_offset DAY);
            WHEN 'weekly' THEN
                SET cost_execution_date = DATE_SUB(history_execution, INTERVAL relative_offset WEEK);
            WHEN 'monthly' THEN
                SET cost_execution_date = DATE_SUB(history_execution, INTERVAL relative_offset MONTH);
            WHEN 'quarterly' THEN
                SET cost_execution_date = DATE_SUB(history_execution, INTERVAL relative_offset * 3 MONTH);
            WHEN 'biannually' THEN
                SET cost_execution_date = DATE_SUB(history_execution, INTERVAL relative_offset * 6 MONTH);
            WHEN 'yearly' THEN
                SET cost_execution_date = DATE_SUB(history_execution, INTERVAL relative_offset YEAR);
            END CASE;
    END IF;
