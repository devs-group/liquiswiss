...
func TestListOrganisations_OnlyShowsOwnMemberships(t *testing.T) {
	...
	orgsA, totalA, err := env.APIService.ListOrganisations(context.Background(), env.UserA.ID, 1, 100)
	...
	orgsB, totalB, err := env.APIService.ListOrganisations(context.Background(), env.UserB.ID, 1, 100)
	require.NoError(t, err)
...
func TestGetOrganisation_MembershipRequired(t *testing.T) {
	...
	orgA, err := env.APIService.GetOrganisation(context.Background(), env.UserA.ID, env.OrgA.ID)
	require.NoError(t, err)
	...
	_, err = env.APIService.GetOrganisation(context.Background(), env.UserA.ID, env.OrgB.ID)
	require.Error(t, err)
	require.ErrorIs(t, err, sql.ErrNoRows)

	// User B can get their own organisation
	orgB, err := env.APIService.GetOrganisation(context.Background(), env.UserB.ID, env.OrgB.ID)
	require.NoError(t, err)
	...
	_, err = env.APIService.GetOrganisation(context.Background(), env.UserB.ID, env.OrgA.ID)
	require.Error(t, err)
	require.ErrorIs(t, err, sql.ErrNoRows)
}
...
func TestUpdateOrganisation_MembershipRequired(t *testing.T) {
	...
	_, err := env.APIService.UpdateOrganisation(context.Background(), models.UpdateOrganisation{
		Name: &newNameA,
	}, env.UserA.ID, env.OrgA.ID)
	require.NoError(t, err)

	// Verify the update worked
	updatedOrgA, err := env.APIService.GetOrganisation(context.Background(), env.UserA.ID, env.OrgA.ID)
	require.NoError(t, err)
	...
	orgAfterAttempt, err := env.APIService.GetOrganisation(context.Background(), env.UserA.ID, env.OrgA.ID)
	require.NoError(t, err)
...
func TestUserSwitchOrganisation_DataChanges(t *testing.T) {
	...
	orgA2, err := env.APIService.CreateOrganisation(context.Background(), models.CreateOrganisation{
	...
	employees, total, err := env.APIService.ListEmployees(context.Background(), env.UserA.ID, 1, 100, "name", "ASC", "", false)
	require.NoError(t, err)
	...
	_, err = env.APIService.GetEmployee(context.Background(), env.UserA.ID, empOrgA2.ID)
