...
func TestListVats_ShowsSystemAndOwnOrg(t *testing.T) {
	...
	vatA1, err := env.APIService.CreateVat(context.Background(), models.CreateVat{Value: 770}, env.UserA.ID)
	require.NoError(t, err)

	vatA2, err := env.APIService.CreateVat(context.Background(), models.CreateVat{Value: 250}, env.UserA.ID)
	require.NoError(t, err)

	// Create org-specific VAT for User B
	vatB1, err := env.APIService.CreateVat(context.Background(), models.CreateVat{Value: 380}, env.UserB.ID)
	require.NoError(t, err)

	// User A should see their own VATs (and any system VATs)
	vatsA, err := env.APIService.ListVats(context.Background(), env.UserA.ID)
	require.NoError(t, err)
	...
	vatsB, err := env.APIService.ListVats(context.Background(), env.UserB.ID)
	require.NoError(t, err)
...
func TestGetVat_CrossOrgIsolation(t *testing.T) {
	...
	vatA, err := env.APIService.CreateVat(context.Background(), models.CreateVat{Value: 770}, env.UserA.ID)
	require.NoError(t, err)

	// User A can get their own VAT
	fetchedVat, err := env.APIService.GetVat(context.Background(), env.UserA.ID, vatA.ID)
	require.NoError(t, err)
	...
	_, err = env.APIService.GetVat(context.Background(), env.UserB.ID, vatA.ID)
	require.Error(t, err)
...
func TestUpdateVat_CrossOrgIsolation(t *testing.T) {
	...
	newValue := int64(800)
	_, err = env.APIService.UpdateVat(context.Background(), models.UpdateVat{Value: &newValue}, env.UserA.ID, vatA.ID)
	require.NoError(t, err)

	// Verify the update worked
	updatedVat, err := env.APIService.GetVat(context.Background(), env.UserA.ID, vatA.ID)
	require.NoError(t, err)
	...
	_, err = env.APIService.UpdateVat(context.Background(), models.UpdateVat{Value: &maliciousValue}, env.UserB.ID, vatA.ID)
	require.Error(t, err)
	require.ErrorIs(t, err, sql.ErrNoRows)

	// Verify VAT was NOT changed by User B
	vatAfterAttempt, err := env.APIService.GetVat(context.Background(), env.UserA.ID, vatA.ID)
	require.NoError(t, err)
...
func TestDeleteVat_CrossOrgIsolation(t *testing.T) {
	...
	err = env.APIService.DeleteVat(context.Background(), env.UserB.ID, vatA.ID)
	// The delete should fail or affect 0 rows

	// Verify VAT still exists and was NOT deleted
	vatAfterDelete, err := env.APIService.GetVat(context.Background(), env.UserA.ID, vatA.ID)
	require.NoError(t, err)
	require.NotNil(t, vatAfterDelete)
	require.Equal(t, vatA.ID, vatAfterDelete.ID)

	// User A can successfully delete their own VAT
	err = env.APIService.DeleteVat(context.Background(), env.UserA.ID, vatA.ID)
	require.NoError(t, err)

	// Verify VAT is now deleted
	_, err = env.APIService.GetVat(context.Background(), env.UserA.ID, vatA.ID)
	require.Error(t, err)
	require.ErrorIs(t, err, sql.ErrNoRows)
}
