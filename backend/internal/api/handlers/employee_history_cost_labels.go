...
func GetEmployeeHistoryCostLabel(dbService db_service.IDatabaseService, c *gin.Context) {
	...
	historyCostLabelID := c.Param("historyCostLabelID")
	if historyCostLabelID == "" {
...
func CreateEmployeeHistoryCostLabel(dbService db_service.IDatabaseService, c *gin.Context) {
	...
	historyCost, err := dbService.GetEmployeeHistoryCostLabel(userID, fmt.Sprint(historyCostLabelID))
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
