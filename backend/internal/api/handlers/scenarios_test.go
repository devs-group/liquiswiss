...
func TestListScenariosIntegration(t *testing.T) {
	...
	names := []string{response.Data[0].Name, response.Data[1].Name}
	assert.ElementsMatch(t, []string{"Baseline", "Upside"}, names)
	assert.Equal(t, int64(2), response.Pagination.TotalCount)
