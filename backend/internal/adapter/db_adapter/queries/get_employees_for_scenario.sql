-- Get employees for a scenario with inheritance support
-- Parameters: scenario_id, user_id
WITH RECURSIVE scenario_hierarchy AS (
    -- Start with target scenario
    SELECT
        id,
        type,
        parent_scenario_id,
...
base_employees AS (
    -- Get employees from horizontal scenarios (base data)
    SELECT
        e.id,
        e.uuid,
        e.scenario_id,
...
employee_with_overrides AS (
    -- Combine base employees with overrides
    SELECT
...
-- Return most specific version (lowest level = current scenario)
