<template>
  <form @submit.prevent class="grid grid-cols-2 gap-2">
    <div class="col-span-2 flex flex-col gap-2">
      <label class="text-sm font-bold" for="name">Prozentualer Wert *</label>
      <InputText @input="onParseAmount"
                 v-model="value" v-bind="valueProps"
                 :class="{'p-invalid': errors['value']?.length}"
                 id="value" type="text" placeholder="Beispiel: 8.1"/>
...
import {AmountToInteger} from "#imports";
...
const { defineField, errors, handleSubmit, meta } = useForm<VatFormData>({
  validationSchema: yup.object({
    value: yup.number()
        ...
        .test('unique', 'Mehrwertsteuer existiert bereits',(value, context) => {
