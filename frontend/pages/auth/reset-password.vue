    ...
    <div v-if="isValid" class="flex flex-col items-center gap-2 w-full max-w-xl mx-auto">
      ...
      <form @submit.prevent class="grid grid-cols-1 gap-2 w-full max-w-sm">
        ...
        <div class="flex justify-end gap-2 col-span-full">
          <Button @click="onFinishResetPassword" severity="info" :disabled="!metaPassword.valid || (metaPassword.valid && !metaPassword.dirty) || isSubmitting" :loading="isSubmitting" label="Passwort setzen" icon="pi pi-save" type="submit"/>
        </div>
      </form>
    </div>
