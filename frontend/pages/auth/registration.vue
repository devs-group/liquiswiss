<template>
  <div class="flex flex-col gap-4 w-full max-w-lg mx-auto">
    ...
    <h1 class="text-2xl font-bold text-center">Registrierung</h1>
      ...
      <template v-if="!isFinished">
        ...
        <form @submit.prevent class="grid grid-cols-1 gap-2 w-full">
          ...
          <div class="flex justify-end gap-2 col-span-full">
            <Button @click="onRegistration" :disabled="!metaPassword.valid || (metaPassword.valid && !metaPassword.dirty) || isSubmitting" :loading="isSubmitting" label="Registrierung starten" icon="pi pi-user" type="submit"/>
          </div>
        </form>
      </template>
      <Message severity="success" v-else>
...
const { defineField: defineFieldEmail, errors: errorsEmail, handleSubmit: handleSubmitEmail, meta: metaPassword, resetForm: resetFormPassword } = useForm({
